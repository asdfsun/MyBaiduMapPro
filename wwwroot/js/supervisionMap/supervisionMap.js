...
var mapObj = {
    ...
    init: function (options) {
        options = $.extend({
            defaultCity: '北京市',
            centerLong: '116.403569',//中心地点经度
            centerLat: '39.924075',//中心地点纬度
            isOnLine: 1,     //1:在线;0:离线
            mapTilesUrl: "", //空值表示默认路径
            customSetting: false,
            customPoint: null,
            setMapZoom: 12,//当前地图级别
            minZoom: 10,//最小地图级别
            maxZoom: 11,//最大地图级别
    ...
    initMap: function () {
        ...
        if (options.isOnLine == 0) {
            if (options.mapTilesUrl.trim().length > 0) {
                ...
                var map = new BMap.Map(options.mapContainerId, { enableMapClick: options.enableMapClick, mapType: tileMapType, minZoom: options.minZoom, maxZoom: options.maxZoom });
            } else {
                var map = new BMap.Map(options.mapContainerId, { enableMapClick: options.enableMapClick, minZoom: options.minZoom, maxZoom: options.maxZoom });
            }
        }
        else {
        ...
        if (options.customSetting) {
            ...
            map.centerAndZoom(new BMap.Point(options.centerLong, options.centerLat),options.setMapZoom);
            //options.defaultCity = map.centerAndZoom(new BMap.Point(options.centerLong, options.centerLat), options.setMapZoom);
        }
        ...
        this.mapClickFunc();
        //this.getAreaPoint(options.defaultCity);
        return this;
    },
    mapClickFunc: function () {
        var mp = this.options.map;
        mp.addEventListener("click", this.options.clickMapCallback);
    ...
    addMarker: function (data,zindex) {
        ...
        marker.addEventListener("click", function () {
            ...
            //    content: "亲耐滴，晚上一起吃个饭吧？戳下面的链接看下地址喔~"
            //}
            //var s = {};
            //s.content = opts;
            //tmp.push(s);
            //that.addInfoWindow(tmp);
    ...
    //获取行政区域边界坐标
    getAreaPoint: function (areaName) {
    ...
    showInfoWindow: function (data) {
        var infoOptions = this.options.showDialogs;
        if (!infoOptions)
        {
            return;
        }
